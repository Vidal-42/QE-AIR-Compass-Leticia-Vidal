# Quality Engineering & AI — CompassUOL | Fase 1 e Fase 2

![Quality Engineering](https://img.shields.io/badge/Quality%20Engineering-CompassUOL-orange)
![Status](https://img.shields.io/badge/status-em%20concluído-green)

---

## Repositório

Este repositório foi criado com o objetivo de documentar, organizar e versionar as atividades desenvolvidas durante o programa **AIR (Academic Internship Program)** da **CompassUOL**, na trilha de **Quality Engineering & AI**.

Aqui estão reunidas as evidências, exercícios e artefatos produzidos ao longo das **6 sprints da Fase 1 (Estágio)**, posteriormente estruturados durante a **Fase 2 (Trainee)** como parte do processo de organização, padronização e versionamento utilizando Git e GitHub.

Este ambiente serve como um repositório central de documentação das práticas, atividades e conhecimentos aplicados ao longo do programa.

---

## Objetivos

- Documentar as atividades realizadas durante a Fase 1 e 2 do Programa de Bolsas
- Centralizar planos de teste, evidências, relatórios e exercícios  
- Aplicar boas práticas de versionamento com Git e GitHub  
- Manter um histórico estruturado das atividades desenvolvidas  
- Organizar os artefatos de Quality Engineering de forma clara e rastreável  

---

## Estrutura

    Documentos/
    ├── Fase 1/
    │   ├── Sprint 1/
    │   ├── Sprint 2/
    │   ├── Sprint 3/
    │   ├── Sprint 4/
    │   ├── Sprint 5/
    │   └── Sprint 6/
    │
    └── Fase 2/
    │   ├── Sprint 1/
    │   ├── Sprint 2/
    │   └── Sprint 3/

Cada sprint contém artefatos relacionados às atividades desenvolvidas, incluindo:

- Planejamento de testes  
- Evidências de execução  
- Relatórios de bugs  
- Testes de API  
- Collections do Postman  
- Mapas mentais e materiais de apoio  

---

## Sprints - Fase-1

- [Sprint-1](https://github.com/Vidal-42/QE-AIR-Compass-Leticia-Vidal/tree/main/Documentos/Fase-1/Sprint-1)
- [Sprint-2](https://github.com/Vidal-42/QE-AIR-Compass-Leticia-Vidal/tree/main/Documentos/Fase-1/Sprint-2)
- [Sprint-3](https://github.com/Vidal-42/QE-AIR-Compass-Leticia-Vidal/tree/main/Documentos/Fase-1/Sprint-3)
- [Sprint-4](https://github.com/Vidal-42/QE-AIR-Compass-Leticia-Vidal/tree/main/Documentos/Fase-1/Sprint-4)
- [Sprint-5](https://github.com/Vidal-42/QE-AIR-Compass-Leticia-Vidal/tree/main/Documentos/Fase-1/Sprint-5)
- [Sprint-6](https://github.com/Vidal-42/QE-AIR-Compass-Leticia-Vidal/tree/main/Documentos/Fase-1/Sprint-6)

## Sprints - Fase-2

- [Sprint-1](https://github.com/Vidal-42/QE-AIR-Compass-Leticia-Vidal/tree/main/Documentos/Fase-2/Sprint-1)
- [Sprint-2](https://github.com/Vidal-42/QE-AIR-Compass-Leticia-Vidal/tree/main/Documentos/Fase-2/Sprint-2)
- [Sprint-3](https://github.com/Vidal-42/QE-AIR-Compass-Leticia-Vidal/tree/main/Documentos/Fase-2/Sprint-3)
  
---
## Contexto

Este repositório foi estruturado durante a **Fase 2 do programa**, com o objetivo de consolidar e versionar os artefatos produzidos anteriormente durante a **Fase 1**, garantindo melhor organização, rastreabilidade e padronização da documentação técnica.

---

**CompassUOL — Academic Internship Program (AIR)**  
**Track: Quality Engineering & AI**








